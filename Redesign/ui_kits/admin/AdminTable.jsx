/* Katharos Admin — report table with actions + inline edit */
                  ...
                  <tr style={{ borderTop: '1px solid #F3F4F6', background: urgent ? 'rgba(254,242,242,0.4)' : '#fff' }}>
                    ...
                    <td style={{ padding: '12px 16px' }}>
                      <div style={{ display: 'flex', gap: 12, alignItems: 'center', flexWrap: 'wrap' }}>
                        <ActionLink color="#006994" onClick={() => {}}>Προβολή</ActionLink>
                        <ActionLink color="#2563EB" onClick={() => setEditingId(editingId === r.id ? null : r.id)}>{editingId === r.id ? 'Ακύρωση' : 'Επεξεργασία'}</ActionLink>
                        {mode === 'pending' && <>
                          <ActionLink color="#6B7C3A" onClick={() => onAction(r.id, 'approve')}>✓ Επαλήθευση</ActionLink>
                    ...
                    <tr style={{ background: '#EFF6FF', borderTop: '1px solid #DBEAFE' }}>
                      <td colSpan={8} style={{ padding: 16 }}>
                        ...
                        <div style={{ display: 'flex', gap: 8 }}>
                          <button onClick={() => setEditingId(null)} style={{ padding: '6px 12px', borderRadius: 8, border: 'none', background: '#006994', color: '#fff', fontSize: 12, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}>Αποθήκευση</button>
