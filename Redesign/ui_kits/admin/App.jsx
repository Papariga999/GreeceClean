/* Katharos Admin — login + dashboard shell */
    ...
    <div style={{ minHeight: '100vh', background: '#F9FAFB', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
      <div style={{ background: '#fff', borderRadius: 24, boxShadow: '0 1px 2px 0 rgb(0 0 0 / 0.05)', border: '1px solid #F3F4F6', padding: 32, width: '100%', maxWidth: 380 }}>
        ...
        <input type="password" value={pw} onChange={e => setPw(e.target.value)} placeholder="••••••••"
          ...
          onFocus={e => e.target.style.boxShadow = '0 0 0 2px #006994'} onBlur={e => e.target.style.boxShadow = 'none'} />
        <button onClick={onLogin} style={{ width: '100%', background: '#006994', color: '#fff', border: 'none', borderRadius: 24, padding: '12px', fontSize: 15, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}>Σύνδεση</button>
      </div>
    </div>
  );
}
        ...
        <tbody>
          {window.MUNICIPALITIES.map(m => (
            <tr key={m.name} style={{ borderTop: '1px solid #F3F4F6' }}>
              ...
              <td style={{ padding: '12px 16px', fontFamily: 'ui-monospace, monospace', fontSize: 12, color: '#006994' }}>{m.email}</td>
      ...
      <div style={{ maxWidth: 1280, margin: '0 auto', padding: '32px 16px' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 32 }}>
          <div>
            ...
            <h1 style={{ fontSize: 30, fontWeight: 700, color: '#006994', margin: 0 }}>Πίνακας Διαχείρισης</h1>
