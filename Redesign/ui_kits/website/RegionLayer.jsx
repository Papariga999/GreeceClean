/* Katharos — Regional / tourism layer (the "destination QR" landing point).
    ...
    <div style={{ background: '#fff' }}>
      ...
      <section style={{ position: 'relative', color: '#fff', overflow: 'hidden' }}>
        ...
        <div style={{ maxWidth: 1040, margin: '0 auto', position: 'relative', padding: '56px 20px 64px' }}>
          {/* demo ribbon */}
          <p style={{ margin: '0 0 22px', fontSize: 11.5, fontFamily: 'ui-monospace, monospace', color: '#A4B16E',
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 36, alignItems: 'center' }}>
            {/* left: message */}
            <div>
              <p style={{ margin: '0 0 14px', fontSize: 13, fontWeight: 600, letterSpacing: '0.14em', textTransform: 'uppercase', color: '#A4B16E' }}>{t.eyebrow}</p>
              ...
              <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
                <button onClick={() => onNav && onNav('report')} style={{ background: '#6B7C3A', color: '#fff', border: 'none',
                  ...
                  onMouseLeave={e => e.currentTarget.style.background = '#6B7C3A'}>📷 {t.reportCta}</button>
              ...
              <div style={{ background: '#fff', borderRadius: 24, padding: 22, width: 230, textAlign: 'center', boxShadow: '0 20px 40px -12px rgba(0,0,0,0.4)' }}>
                <div style={{ display: 'grid', gridTemplateColumns: `repeat(${cells}, 1fr)`, gap: 2, width: 168, height: 168, margin: '0 auto' }}>
                  {Array.from({ length: cells * cells }).map((_, i) => {
                    ...
                    return <div key={i} style={{ background: on ? '#004A6A' : 'transparent', borderRadius: 1 }} />;
                  })}
                </div>
                <p style={{ margin: '14px 0 2px', fontSize: 14, fontWeight: 700, color: '#006994' }}>{t.qrLabel}</p>
      ...
      <section style={{ padding: '48px 20px', background: '#F5F2ED' }}>
        <div style={{ maxWidth: 1040, margin: '0 auto' }}>
          ...
          <div style={{ textAlign: 'center' }}>
            ...
            <button onClick={() => onNav && onNav('partners')} style={{ background: 'none', border: 'none', cursor: 'pointer',
              color: '#006994', fontWeight: 700, fontSize: 15, fontFamily: 'inherit', padding: 0 }}
              onMouseEnter={e => e.currentTarget.style.color = '#005A80'}
              onMouseLeave={e => e.currentTarget.style.color = '#006994'}>{t.partnerCta}</button>
          </div>
        </div>
      </section>
    </div>
