# Developer-Briefing — Katharos Partner-/Sponsorenseite
...
Eine **dreisprachige (EL · EN · DE) Partner-/Sponsorenseite** als Unterseite der bestehenden Katharos-Website, plus alle Einstiegspunkte aus der Bürger-App dorthin. Substanz vor Ask (≈80/20), seriöser, partnerschaftlicher Ton, keine erfundenen Logos/Zahlen.
...
Reihenfolge fix; Register formell. Quelle: `greececlean-partnerseite-copy.md` §5. Im i18n unter dem Key `claims.items` (`{ title, desc }` × 6); Rahmen-Texte: Eyebrow „Warum unterstützenswert" · Heading „Warum Katharos unterstützenswert ist" · Sub „Sechs Gründe, kurz und konkret — das Kernargument für eine Unterstützung."
...
**Claim-Card (Muster, `Partners.jsx` §5):** `window.Card` (weiß, `rounded-2xl`, Hairline-Border, `shadow-sm`) → Kopfzeile mit nummeriertem Badge `01`–`06` (46×46, `radius 14`, BG `#E6EFF3`, Text `#006994`, **Mono**, weight 800) + Titel (`h3`, 18px, bold, `#006994`) → Stützzeile (14.5px, `#4B5563`). Hover-Lift identisch zu §6. Sektionsfläche **Sea Mist `#F5F2ED`** (Rhythmus §4 Blau → §5 Sea Mist → §6 Weiß). Darunter der **leere Partner-Logo-Bereich** (i18n `claims.partnersLabel` / `claims.partnersNote`) — erst mit echten Logos befüllen.
...
- **Error:** freundlicher Text + Retry + direkte E-Mail (`partners@katharos.gr`), nie technische Fehlermeldungen.
...
### Backend
- Versand über die **bestehende Resend-/E-Mail-Pipeline** an ein internes Postfach (z. B. `partners@katharos.gr`).
...
- **Aegean Blue** `#006994` (Primär), dunkleres Blau `#005A80` / `#004A6A`, **Eco Green** `#6B7C3A` (Hover `#5A6830`), Sea Mist `#F5F2ED`, Off-White `#F9FAFB`.
