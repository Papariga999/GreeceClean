/* GreeceClean · User-flow diagram — the viral accountability loop */

function FlowNode({ x, y, w, h, icon, title, sub, tone }) {
  const { T } = window;
  const tones = {
    entry: { bg: '#EAF2FC', bd: '#BBD9F7', fg: T.primary },
    core: { bg: T.primary, bd: T.primary, fg: '#fff' },
    action: { bg: '#fff', bd: T.line2, fg: T.ink },
    loop: { bg: '#EAF7EC', bd: '#BFE0C6', fg: '#2E7D34' },
    ...
    <div style={{ width: W, height: H, background: '#fff', position: 'relative', fontFamily: "'Inter', sans-serif", padding: 0 }}>
      ...
      <svg width={W} height={H} style={{ position: 'absolute', inset: 0 }}>
        <defs>
          ...
          <marker id="ahg" markerWidth="9" markerHeight="9" refX="7" refY="4.5" orient="auto"><path d="M0,0 L9,4.5 L0,9 z" fill="#39B24A" /></marker>
        </defs>
        {/* A->B */}
        ...
        <path d="M860,340 C880,470 880,528 120,528 L120,346" fill="none" stroke="#39B24A" strokeWidth="2.2" strokeDasharray="7 5" markerEnd="url(#ahg)" />
      ...
      <div style={{ position: 'absolute', left: 250, top: 500, fontSize: 11.5, fontWeight: 700, color: '#39B24A' }}>↻ νέοι άνθρωποι μπαίνουν στον βρόχο</div>
    </div>
  );
}
