/* GreeceClean · Screen 6 — Municipality Scorecard (shareable accountability page) */
  ...
  return (
    <window.PhoneFrame height={690} contentBg={T.bg}>
      ...
      <div style={{ flex: 1, overflow: 'hidden', padding: 16, display: 'flex', flexDirection: 'column', gap: 14 }}>
        {/* hero */}
        <div style={{ background: 'linear-gradient(140deg, #0D6FDB, #0B57AD)', borderRadius: 20, padding: 18, color: '#fff' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'flex-end', gap: 7, height: 90 }}>
            {bars.map((b, i) => (
              <div key={i} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
                <div style={{ width: '100%', height: b + '%', background: i === bars.length - 1 ? T.action : T.primary200 || '#BBD9F7', borderRadius: '4px 4px 0 0', opacity: i === bars.length - 1 ? 1 : 0.5 }} />
              </div>
            ))}
          </div>
