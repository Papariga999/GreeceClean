/* GreeceClean — shared UI primitives + chrome (Header, Footer)
...
function Header({ onNav, active, lang: langProp, setLang: setLangProp }) {
  ...
  const link = (key, label) => (
    <button onClick={() => onNav(key)} style={{
      ...
      onMouseEnter={e => e.currentTarget.style.color = '#BFE0C6'}
      onMouseLeave={e => e.currentTarget.style.color = '#fff'}>{label}</button>
  );
  return (
    <header style={{ background: '#0D6FDB', color: '#fff', boxShadow: '0 4px 6px -1px rgb(0 0 0 / 0.1)' }}>
      <div style={{ maxWidth: 1152, margin: '0 auto', padding: '12px 16px',
        ...
        <nav style={{ display: 'flex', alignItems: 'center', gap: 22 }}>
          ...
          <button onClick={() => onNav('report')} style={{
            background: '#39B24A', color: '#fff', border: 'none', cursor: 'pointer',
            ...
            onMouseLeave={e => e.currentTarget.style.background = '#39B24A'}>{nav.report}</button>
  ...
  return (
    <footer style={{ borderTop: '1px solid #F3F4F6', background: '#fff', marginTop: 0 }}>
      <div style={{ maxWidth: 896, margin: '0 auto', padding: '32px 16px',
        ...
        <p style={{ margin: 0 }}>© 2026 GreeceClean — Helping keep Greece clean</p>
        <nav style={{ display: 'flex', gap: 20, flexWrap: 'wrap' }}>
          {links.map(x => (
            <a key={x.label} href={x.key ? ('#' + x.key) : '#'}
              onClick={e => { e.preventDefault(); if (x.key && onNav) onNav(x.key); }}
              style={{ color: x.strong ? '#0D6FDB' : '#9CA3AF', textDecoration: 'none', fontWeight: x.strong ? 600 : 400 }}
              onMouseEnter={e => e.currentTarget.style.color = x.strong ? '#0B57AD' : '#4B5563'}
              onMouseLeave={e => e.currentTarget.style.color = x.strong ? '#0D6FDB' : '#9CA3AF'}>{x.label}</a>
          ))}
        </nav>
      </div>
    </footer>
...
function SupportBanner({ lang, onNav }) {
  ...
  return (
    <section style={{ background: '#F2F7FB', borderTop: '1px solid #E0EAF4' }}>
      <div style={{ maxWidth: 1024, margin: '0 auto', padding: '28px 20px', display: 'flex',
        ...
        <div style={{ minWidth: 240, flex: 1 }}>
          <p style={{ margin: '0 0 4px', fontSize: 18, fontWeight: 700, color: '#0D6FDB', letterSpacing: '-0.01em' }}>{b.title}</p>
          <p style={{ margin: 0, fontSize: 14, color: '#4B5563', lineHeight: 1.5 }}>{b.sub}</p>
        </div>
        <button onClick={() => onNav && onNav('partners')} style={{
          background: '#39B24A', color: '#fff', border: 'none', cursor: 'pointer', flexShrink: 0,
          padding: '12px 24px', borderRadius: 16, fontSize: 15, fontWeight: 600, fontFamily: 'inherit',
          transition: 'background .2s' }}
          onMouseEnter={e => e.currentTarget.style.background = '#2E8C3B'}
          onMouseLeave={e => e.currentTarget.style.background = '#39B24A'}>{b.cta}</button>
      </div>
    </section>
  );
}
