/* GreeceClean — Partners / Sponsors page (subpage). All 9 sections, mobile-first
...
function PHolder({ label, height, radius = 24, style }) {
  return (
    <div style={{ height, borderRadius: radius, position: 'relative', overflow: 'hidden',
      ...
      border: '1px solid #E0EAF4', display: 'flex', alignItems: 'center', justifyContent: 'center', ...style }}>
  ...
  return (
    <div style={{ textAlign: align, maxWidth: align === 'center' ? max : 'none', margin: align === 'center' ? '0 auto' : 0 }}>
      <p style={{ margin: '0 0 12px', fontSize: 13, fontWeight: 600, letterSpacing: '0.12em', textTransform: 'uppercase',
        color: light ? '#9FE3AC' : '#6B7280' }}>{eyebrow}</p>
      <h2 style={{ margin: 0, fontSize: 'clamp(26px, 3.4vw, 38px)', fontWeight: 800, lineHeight: 1.15,
        letterSpacing: '-0.02em', color: light ? '#fff' : '#0D6FDB' }}>{heading}</h2>
...
function Partners({ lang, onNav }) {
  ...
  const heroBtn = (primary, children, onClick) => (
    <button onClick={onClick} style={{
      background: primary ? '#39B24A' : 'rgba(255,255,255,0.10)', color: '#fff',
      ...
      onMouseEnter={e => e.currentTarget.style.background = primary ? '#2E8C3B' : 'rgba(255,255,255,0.2)'}
      onMouseLeave={e => e.currentTarget.style.background = primary ? '#39B24A' : 'rgba(255,255,255,0.10)'}>{children}</button>
  );
    ...
    <div style={{ background: '#fff' }}>
      ...
      <section style={{ position: 'relative', color: '#fff', overflow: 'hidden' }}>
        ...
        <div style={{ ...wrap(880), position: 'relative', padding: '108px 20px 100px', textAlign: 'center' }}>
          <p style={{ margin: '0 0 18px', fontSize: 13, fontWeight: 600, letterSpacing: '0.16em', textTransform: 'uppercase', color: '#9FE3AC' }}>{t.hero.eyebrow}</p>
          <h1 style={{ margin: '0 0 20px', fontSize: 'clamp(34px, 5.4vw, 58px)', fontWeight: 800, lineHeight: 1.08, letterSpacing: '-0.025em', color: '#fff' }}>
            {t.hero.pre}<span style={{ color: '#9FE3AC' }}>{t.hero.hi}</span>{t.hero.post}
          </h1>
            ...
            {t.problem.stats.map((s, i) => (
              <window.Card key={i} style={{ padding: '28px 26px' }}>
                <div style={{ display: 'flex', alignItems: 'baseline', gap: 8, flexWrap: 'wrap' }}>
                  <span style={{ fontSize: 40, fontWeight: 800, color: '#0D6FDB', letterSpacing: '-0.02em', lineHeight: 1 }}>{s.value}</span>
                  {s.sub && <span style={{ fontSize: 15, fontWeight: 700, color: '#0B57AD' }}>{s.sub}</span>}
                </div>
        ...
        <div style={wrap(W)}>
            ...
            {t.solution.steps.map((s, i) => (
              <window.Card key={i} style={{ padding: '28px 26px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 14 }}>
                  <span style={{ width: 52, height: 52, borderRadius: 9999, background: '#F2F7FB', display: 'flex',
                    alignItems: 'center', justifyContent: 'center', fontSize: 24, flexShrink: 0 }}>{s.icon}</span>
                  <span style={{ fontSize: 13, fontWeight: 800, color: '#39B24A', fontFamily: 'ui-monospace, monospace' }}>0{i + 1}</span>
                </div>
                <h3 style={{ margin: '0 0 8px', fontSize: 19, fontWeight: 700, color: '#0D6FDB' }}>{s.title}</h3>
          ...
          <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', justifyContent: 'center' }}>
            {t.solution.features.map(ft => (
              <span key={ft} style={{ display: 'inline-flex', alignItems: 'center', gap: 7, background: '#F2F7FB',
                color: '#0B57AD', borderRadius: 9999, padding: '8px 16px', fontSize: 13.5, fontWeight: 600 }}>
                <span style={{ color: '#39B24A', fontWeight: 800 }}>✓</span>{ft}</span>
            ))}
          </div>
        </div>
      </section>
      ...
      {/* ================================================ 4 · MISSION & VISION */}
      <section style={{ padding: sectionPad, background: 'linear-gradient(155deg, #0D6FDB, #0B3F7E)', color: '#fff' }}>
        <div style={wrap(W)}>
          <p style={{ textAlign: 'center', margin: '0 0 4px', fontSize: 13, fontWeight: 600, letterSpacing: '0.12em', textTransform: 'uppercase', color: '#9FE3AC' }}>{t.mv.eyebrow}</p>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: 28, marginTop: 32 }}>
            {[[t.mv.missionLabel, t.mv.mission], [t.mv.visionLabel, t.mv.vision]].map(([label, body]) => (
              <div key={label} style={{ background: 'rgba(255,255,255,0.07)', border: '1px solid rgba(255,255,255,0.16)',
                ...
                <p style={{ margin: '0 0 16px', fontSize: 13, fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase', color: '#9FE3AC' }}>{label}</p>
      ...
      <section style={{ padding: sectionPad, background: '#F2F7FB' }}>
        <div style={wrap(W)}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: 22, marginTop: 44 }}>
            {t.claims.items.map((c, i) => (
              <window.Card key={i} style={{ padding: '28px 26px', display: 'flex', flexDirection: 'column', transition: 'box-shadow .2s, transform .2s' }}
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 14 }}>
                  <span style={{ width: 46, height: 46, borderRadius: 14, background: '#EAF2FC', color: '#0D6FDB', display: 'flex',
                  ...
                  <h3 style={{ margin: 0, fontSize: 18, fontWeight: 700, color: '#0D6FDB', lineHeight: 1.25 }}>{c.title}</h3>
                </div>
            ...
            {t.why.cards.map((c, i) => (
              <window.Card key={i} style={{ padding: '28px 26px', display: 'flex', flexDirection: 'column', transition: 'box-shadow .2s, transform .2s' }}
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16 }}>
                  <span style={{ width: 48, height: 48, borderRadius: 14, background: '#F2F7FB', display: 'flex',
                    alignItems: 'center', justifyContent: 'center', fontSize: 24, flexShrink: 0 }}>{c.icon}</span>
                  <h3 style={{ margin: 0, fontSize: 18, fontWeight: 700, color: '#0D6FDB', lineHeight: 1.2 }}>{c.title}</h3>
                </div>
                <ul style={{ margin: '0 0 20px', padding: 0, listStyle: 'none', display: 'flex', flexDirection: 'column', gap: 10, flex: 1 }}>
                  {c.bullets.map((b, j) => (
                    <li key={j} style={{ display: 'flex', gap: 10, fontSize: 14, color: '#4B5563', lineHeight: 1.55 }}>
                      <span style={{ color: '#39B24A', fontWeight: 800, flexShrink: 0 }}>✓</span><span>{b}</span>
                    </li>
                  ))}
                </ul>
                <button onClick={() => scrollTo(contactRef)} style={{ alignSelf: 'flex-start', background: 'none', border: 'none',
                  color: '#0D6FDB', fontWeight: 700, fontSize: 14, cursor: 'pointer', fontFamily: 'inherit', padding: 0,
                  display: 'inline-flex', alignItems: 'center', gap: 6 }}
                  onMouseEnter={e => e.currentTarget.style.color = '#0B57AD'}
                  onMouseLeave={e => e.currentTarget.style.color = '#0D6FDB'}>{t.why.cta} <span aria-hidden="true">→</span></button>
                {i === 2 && t.why.regionLink && (
                  <button onClick={() => onNav && onNav('region')} style={{ alignSelf: 'flex-start', marginTop: 10, background: 'none',
                    border: 'none', color: '#2E8C3B', fontWeight: 600, fontSize: 13, cursor: 'pointer', fontFamily: 'inherit', padding: 0 }}
                    onMouseEnter={e => e.currentTarget.style.color = '#1F6B2A'}
                    onMouseLeave={e => e.currentTarget.style.color = '#2E8C3B'}>{t.why.regionLink}</button>
                )}
              </window.Card>
            ))}
          </div>
      ...
      <section style={{ padding: sectionPad, background: '#F9FAFB' }}>
        <div style={wrap(W)}>
            ...
            {t.offer.items.map((o, i) => (
              <window.Card key={i} style={{ padding: '24px 22px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 11, marginBottom: 10 }}>
                  <span style={{ width: 40, height: 40, borderRadius: 11, background: '#ECF7EE', display: 'flex',
          ...
          <div style={{ background: '#031C36', borderRadius: 24, padding: '36px 32px', color: '#fff' }}>
            <div style={{ maxWidth: 760, margin: '0 auto', textAlign: 'center' }}>
              <div style={{ fontSize: 30, marginBottom: 10 }}>🛡️</div>
              <h3 style={{ margin: '0 0 8px', fontSize: 22, fontWeight: 800, color: '#fff' }}>{t.offer.transHeading}</h3>
              <p style={{ margin: '0 0 24px', fontSize: 15, color: '#A6C7F7' }}>{t.offer.transSub}</p>
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: 16, maxWidth: 880, margin: '0 auto' }}>
              {t.offer.transPoints.map((p, i) => (
                <div key={i} style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.12)',
                  ...
                  <span style={{ color: '#5DBF6D', fontWeight: 800, fontSize: 16, flexShrink: 0 }}>✓</span>
      ...
      <section ref={contactRef} style={{ padding: sectionPad, background: '#fff' }}>
        <div style={wrap(PROSE)}>
          ...
          <p style={{ textAlign: 'center', margin: '14px auto 0', fontSize: 14, color: '#4B5563' }}>
            {t.contact.altIntro}{' '}
            <a href={'mailto:' + window.PARTNERS_EMAIL} style={{ color: '#0D6FDB', fontWeight: 600, textDecoration: 'none', fontFamily: 'ui-monospace, monospace' }}>{window.PARTNERS_EMAIL}</a>
            <span style={{ color: '#D1D5DB', margin: '0 10px' }}>·</span>
            <a href="#" onClick={e => e.preventDefault()} style={{ color: '#0D6FDB', fontWeight: 600, textDecoration: 'none' }}>{t.contact.altLinkedin}</a>
          </p>
        </div>
      </section>
