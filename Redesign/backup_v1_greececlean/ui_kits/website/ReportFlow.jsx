/* GreeceClean — Report Flow (4-step wizard: Category → Photos → Location → Submit) */
      ...
      {steps.map((s, i) => (
        <React.Fragment key={s}>
          <div title={s} style={{ width: 32, height: 32, borderRadius: 9999, display: 'flex',
            ...
            background: i < idx ? '#39B24A' : i === idx ? '#0D6FDB' : '#E5E7EB',
            transition: 'background .2s' }}>{i < idx ? '✓' : i + 1}</div>
          {i < steps.length - 1 && (
            <div style={{ width: 32, height: 2, background: i < idx ? '#39B24A' : '#E5E7EB' }} />
          )}
        </React.Fragment>
      ))}
    </div>
...
function ReportFlow({ onTracking, onNav, lang }) {
  ...
  const navRow = (back, next, nextLabel, nextDisabled, nextAction, actionBtn) => (
    <div style={{ display: 'flex', gap: 12, marginTop: 8 }}>
      ...
      <button onClick={nextAction} disabled={nextDisabled} style={{ flex: 1, border: 'none', borderRadius: 24,
        padding: '12px', color: '#fff', background: actionBtn ? '#39B24A' : '#0D6FDB', cursor: nextDisabled ? 'not-allowed' : 'pointer',
  ...
  return (
    <div style={{ maxWidth: 512, margin: '0 auto', padding: '32px 16px', minHeight: 480 }}>
      ...
      {step === 0 && (
        <div>
          <h2 style={{ fontSize: 24, fontWeight: 700, color: '#0D6FDB', margin: '0 0 4px' }}>Category</h2>
      ...
      {step === 1 && (
        <div>
          <h2 style={{ fontSize: 24, fontWeight: 700, color: '#0D6FDB', margin: '0 0 4px' }}>Photo</h2>
          ...
          {photos.length < 3 && (
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 12 }}>
              {[['📷', 'Camera', '#4D8FEF', '#EAF2FC', '#0D6FDB'], ['🖼️', 'Photo Library', '#D1D5DB', '#F9FAFB', '#4B5563']].map(([icon, label, bc, hb, fg], k) => (
          ...
          {photos.length > 0 && (
            <p style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12, color: '#39B24A', fontWeight: 500, margin: '0 0 16px' }}>📍 Location found</p>
          )}
      ...
      {step === 2 && (
        <div>
          <h2 style={{ fontSize: 24, fontWeight: 700, color: '#0D6FDB', margin: '0 0 4px' }}>Location</h2>
      ...
      {step === 3 && (
        <div>
          <h2 style={{ fontSize: 24, fontWeight: 700, color: '#0D6FDB', margin: '0 0 20px' }}>Almost done!</h2>
          ...
          <textarea value={desc} onChange={e => setDesc(e.target.value.slice(0, 500))} rows={4}
            ...
            onFocus={e => e.target.style.boxShadow = '0 0 0 2px #0D6FDB'}
      ...
      {step === 4 && (
        <div style={{ textAlign: 'center', paddingTop: 16 }}>
          ...
          <h2 style={{ fontSize: 24, fontWeight: 700, color: '#0D6FDB', margin: '0 0 12px' }}>Thank you for making Greece cleaner! 🌿</h2>
          ...
          <window.Card style={{ background: '#F9FAFB', marginBottom: 24, textAlign: 'left' }}>
            ...
            <p style={{ fontSize: 14, fontFamily: 'ui-monospace, monospace', color: '#0D6FDB', wordBreak: 'break-all', margin: '0 0 16px' }}>greececlean.gr/r/ab12cd34ef56</p>
            <button onClick={onTracking} style={{ width: '100%', padding: '10px', borderRadius: 24, border: 'none',
              background: '#0D6FDB', color: '#fff', fontSize: 14, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}>View tracking page →</button>
          </window.Card>
          <button onClick={() => { setStep(0); setCategory(null); setPhotos([]); setDesc(''); }}
            style={{ width: '100%', padding: '12px', borderRadius: 24, border: 'none', background: '#0D6FDB',
          ...
          {onNav && (
            <div style={{ marginTop: 28, paddingTop: 20, borderTop: '1px solid #F3F4F6' }}>
              <button onClick={() => onNav('partners')} style={{ background: 'none', border: 'none', cursor: 'pointer',
                ...
                onMouseEnter={e => e.currentTarget.style.color = '#0D6FDB'}
