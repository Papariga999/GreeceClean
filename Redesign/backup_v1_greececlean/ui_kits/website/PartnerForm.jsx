/* GreeceClean — Partner contact form (B2B). Normal / loading / success / error
...
function PartnerForm({ lang, onNav }) {
  ...
  const focusOn = (e, bad) => { if (!bad) e.target.style.boxShadow = '0 0 0 2px #0D6FDB'; };
  ...
  if (status === 'success') {
    return (
      <window.Card style={{ padding: '44px 28px', textAlign: 'center', maxWidth: 560, margin: '0 auto', position: 'relative' }}>
        ...
        <h3 style={{ margin: '0 0 10px', fontSize: 22, fontWeight: 800, color: '#0D6FDB' }}>{t.success.title}</h3>
        <p style={{ margin: '0 auto 28px', maxWidth: 380, color: '#4B5563', fontSize: 15, lineHeight: 1.6 }}>{t.success.body}</p>
        <div style={{ display: 'flex', gap: 12, justifyContent: 'center', flexWrap: 'wrap' }}>
          <button onClick={() => onNav && onNav('map')} style={{ background: '#0D6FDB', color: '#fff', border: 'none',
            borderRadius: 16, padding: '11px 22px', fontSize: 14, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}>{t.success.backMap}</button>
          <button onClick={() => onNav && onNav('home')} style={{ background: '#fff', color: '#0B57AD', border: '1px solid #D2E3FB',
    ...
    return (
      <window.Card style={{ padding: '44px 28px', textAlign: 'center', maxWidth: 560, margin: '0 auto', position: 'relative' }}>
        ...
        <div style={{ width: 64, height: 64, borderRadius: 9999, background: '#FEE2E2', display: 'flex',
          alignItems: 'center', justifyContent: 'center', margin: '0 auto 20px', fontSize: 30 }}>!</div>
        <h3 style={{ margin: '0 0 10px', fontSize: 22, fontWeight: 800, color: '#0D6FDB' }}>{t.error.title}</h3>
        <p style={{ margin: '0 auto 12px', maxWidth: 400, color: '#4B5563', fontSize: 15, lineHeight: 1.6 }}>{t.error.body}</p>
        <a href={'mailto:' + EMAIL} style={{ display: 'inline-block', marginBottom: 24, color: '#0D6FDB', fontWeight: 600,
          fontSize: 14, fontFamily: 'ui-monospace, monospace', textDecoration: 'none' }}>{EMAIL}</a>
        <div>
          <button onClick={reset} style={{ background: '#0D6FDB', color: '#fff', border: 'none',
    ...
    <window.Card style={{ padding: '28px 26px', maxWidth: 560, margin: '0 auto', position: 'relative' }}>
      ...
      <form onSubmit={submit} noValidate style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        <div>
          <label htmlFor="pf-consent" style={{ display: 'flex', gap: 10, alignItems: 'flex-start', cursor: 'pointer', fontSize: 13, color: '#4B5563', lineHeight: 1.5 }}>
            <input id="pf-consent" type="checkbox" checked={vals.consent} disabled={loading}
              ...
              style={{ width: 18, height: 18, marginTop: 1, accentColor: '#0D6FDB', flexShrink: 0, cursor: 'pointer' }} />
            <span>{f.consent}{' '}
              <a href="#" onClick={e => e.preventDefault()} style={{ color: '#0D6FDB', fontWeight: 600, textDecoration: 'underline' }}>{f.consentLink}</a>
            </span>
          </label>
        ...
        <button type="submit" disabled={loading} style={{
          marginTop: 4, background: loading ? '#2E8C3B' : '#39B24A', color: '#fff', border: 'none',
          ...
          onMouseEnter={e => { if (!loading) e.currentTarget.style.background = '#2E8C3B'; }}
          onMouseLeave={e => { if (!loading) e.currentTarget.style.background = '#39B24A'; }}>
      ...
      <div style={{ display: 'flex', gap: 2, background: '#F3F4F6', borderRadius: 9999, padding: 2 }}>
        {opts.map(([k, label]) => (
          <button key={k} type="button" onClick={() => (k === 'idle' ? reset() : setStatus(k))} style={{
            ...
            background: cur === k ? '#fff' : 'transparent', color: cur === k ? '#0D6FDB' : '#9CA3AF',
